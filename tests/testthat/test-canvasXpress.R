...
test_that("canvasXpress_obj properly instantiates", {
    smp_attr <- list(c("r1", "smp_attr_1", "smp_attr_2"))
    var_attr <- list(c("c1", "var_attr_1", "var_attr_2"))
    vals <- data.frame(r2 = c(1,3), r3 = c(2,4))
    vars <- c("r2", "r3")
    smps <- c("c2", "c3")
    cvX <- canvasXpress_obj$new(vars = vars, smps = smps, vals = vals,
                                smp_att = smp_attr, var_att = var_attr)
    expected_y <- list(vars = c("r2", "r3"), smps = c("c2", "c3"),
                       data = data.frame(r2 = c(1,3), r3 = c(2, 4),
                                         row.names = 1:2))
    expected_x <- list(c1 = c("var_attr_1", "var_attr_2"))
    expected_z <- list(r1 = c("smp_attr_1", "smp_attr_2"))
    testthat::expect_equal(cvX$data_structure$y, expected_y)
    testthat::expect_equal(cvX$data_structure$x, expected_x)
    testthat::expect_equal(cvX$data_structure$z, expected_z)
})

test_that("canvasXpress_obj properly instantiates with attributes of length
           greater than one", {
    smp_attr <- list(c("r1", "smp_attr_1", "smp_attr_2"), c("r4", "smp_attr_3",
                       "smp_attr_4"))
    var_attr <- list(c("c1", "var_attr_1", "var_attr_2"), c("c4", "var_attr_3",
                       "var_attr_4"))
    vals <- data.frame(r2 = c(1,3), r3 = c(2,4))
    vars <- c("r2", "r3")
    smps <- c("c2", "c3")
    expected_y <- list(vars = c("r2", "r3"), smps = c("c2", "c3"),
                       data = data.frame(r2 = c(1,3), r3 = c(2, 4),
                                         row.names = 1:2))
    expected_x <- list(c1 = c("var_attr_1", "var_attr_2"),
                       c4 = c("var_attr_3", "var_attr_4"))
    expected_z <- list(r1 = c("smp_attr_1", "smp_attr_2"),
                       r4 = c("smp_attr_3", "smp_attr_4"))
    cvX <- canvasXpress_obj$new(vars = vars, smps = smps, vals = vals,
                                smp_att = smp_attr, var_att = var_attr)
    testthat::expect_equal(cvX$data_structure$y, expected_y)
    testthat::expect_equal(cvX$data_structure$x, expected_x)
